...
package body SData_Core.Evaluator.Distrib_Fns is
   ...
   begin
      ...
      if N >= 3 then
         ...
         return Num_Result (SData_Core.Statistics.Normal_PDF (Convert_To_Real (Vals.Element (1)),
                                                         Convert_To_Real (Vals.Element (2)),
                                                         Convert_To_Real (Vals.Element (3))));
      else
         if not Has_Args (Vals, 1) then return (Kind => Val_Missing); end if;
         return Num_Result (SData_Core.Statistics.Normal_PDF (Convert_To_Real (Vals.Element (1)), 0.0, 1.0));
      end if;
   end Handle_ZDF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Binomial_PMF (Convert_To_Real (Vals.Element (1)),
                                                        Convert_To_Real (Vals.Element (2)),
                                                        Convert_To_Real (Vals.Element (3))));
   end Handle_NDF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Uniform_PDF (Convert_To_Real (Vals.Element (1)),
                                                       Convert_To_Real (Vals.Element (2)),
                                                       Convert_To_Real (Vals.Element (3))));
   end Handle_UDF;
   ...
   begin
      ...
      return Num_Result (SData_Core.Statistics.Exponential_PDF (Convert_To_Real (Vals.Element (1)),
                                                           Convert_To_Real (Vals.Element (2))));
   end Handle_EDF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Beta_PDF (Convert_To_Real (Vals.Element (1)),
                                                    Convert_To_Real (Vals.Element (2)),
                                                    Convert_To_Real (Vals.Element (3))));
   end Handle_BDF;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Poisson_PMF (Convert_To_Real (Vals.Element (1)),
                                                       Convert_To_Real (Vals.Element (2))));
   end Handle_PDF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Gamma_PDF (Convert_To_Real (Vals.Element (1)),
                                                     Convert_To_Real (Vals.Element (2)),
                                                     Convert_To_Real (Vals.Element (3))));
   end Handle_GDF;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Chi_Square_PDF (Convert_To_Real (Vals.Element (1)),
                                                          Convert_To_Real (Vals.Element (2))));
   end Handle_XDF;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Student_T_PDF (Convert_To_Real (Vals.Element (1)),
                                                         Convert_To_Real (Vals.Element (2))));
   end Handle_TDF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.F_PDF (Convert_To_Real (Vals.Element (1)),
                                                 Convert_To_Real (Vals.Element (2)),
                                                 Convert_To_Real (Vals.Element (3))));
   end Handle_FDF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Weibull_PDF (Convert_To_Real (Vals.Element (1)),
                                                       Convert_To_Real (Vals.Element (2)),
                                                       Convert_To_Real (Vals.Element (3))));
   end Handle_WDF;
   ...
   begin
      ...
      if N >= 3 then
         if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
         return Num_Result (SData_Core.Statistics.Normal_CDF (Convert_To_Real (Vals.Element (1)),
                                                         Convert_To_Real (Vals.Element (2)),
                                                         Convert_To_Real (Vals.Element (3))));
      else
         if not Has_Args (Vals, 1) then return (Kind => Val_Missing); end if;
         return Num_Result (SData_Core.Statistics.Normal_CDF (Convert_To_Real (Vals.Element (1)), 0.0, 1.0));
      end if;
   end Handle_ZCF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Binomial_CDF (Convert_To_Real (Vals.Element (1)),
                                                        Convert_To_Real (Vals.Element (2)),
                                                        Convert_To_Real (Vals.Element (3))));
   end Handle_NCF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Uniform_CDF (Convert_To_Real (Vals.Element (1)),
                                                       Convert_To_Real (Vals.Element (2)),
                                                       Convert_To_Real (Vals.Element (3))));
   end Handle_UCF;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Exponential_CDF (Convert_To_Real (Vals.Element (1)),
                                                           Convert_To_Real (Vals.Element (2))));
   end Handle_ECF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Beta_CDF (Convert_To_Real (Vals.Element (1)),
                                                    Convert_To_Real (Vals.Element (2)),
                                                    Convert_To_Real (Vals.Element (3))));
   end Handle_BCF;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Poisson_CDF (Convert_To_Real (Vals.Element (1)),
                                                       Convert_To_Real (Vals.Element (2))));
   end Handle_PCF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Gamma_CDF (Convert_To_Real (Vals.Element (1)),
                                                     Convert_To_Real (Vals.Element (2)),
                                                     Convert_To_Real (Vals.Element (3))));
   end Handle_GCF;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Chi_Square_CDF (Convert_To_Real (Vals.Element (1)),
                                                          Convert_To_Real (Vals.Element (2))));
   end Handle_XCF;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Student_T_CDF (Convert_To_Real (Vals.Element (1)),
                                                         Convert_To_Real (Vals.Element (2))));
   end Handle_TCF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.F_CDF (Convert_To_Real (Vals.Element (1)),
                                                 Convert_To_Real (Vals.Element (2)),
                                                 Convert_To_Real (Vals.Element (3))));
   end Handle_FCF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Weibull_CDF (Convert_To_Real (Vals.Element (1)),
                                                       Convert_To_Real (Vals.Element (2)),
                                                       Convert_To_Real (Vals.Element (3))));
   end Handle_WCF;
   ...
   begin
      if not Has_Args (Vals, 1) then return (Kind => Val_Missing); end if;
      return Num_Result (1.0 / (1.0 + Exp (-Convert_To_Real (Vals.Element (1)))));
   end Handle_LCF;
   ...
   begin
      ...
      if N >= 3 then
         if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
         return Num_Result (SData_Core.Statistics.Normal_IDF (Convert_To_Real (Vals.Element (1)),
                                                         Convert_To_Real (Vals.Element (2)),
                                                         Convert_To_Real (Vals.Element (3))));
      else
         if not Has_Args (Vals, 1) then return (Kind => Val_Missing); end if;
         return Num_Result (SData_Core.Statistics.Normal_IDF (Convert_To_Real (Vals.Element (1)), 0.0, 1.0));
      end if;
   end Handle_ZIF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Binomial_IDF (Convert_To_Real (Vals.Element (1)),
                                                        Convert_To_Real (Vals.Element (2)),
                                                        Convert_To_Real (Vals.Element (3))));
   end Handle_NIF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Uniform_IDF (Convert_To_Real (Vals.Element (1)),
                                                       Convert_To_Real (Vals.Element (2)),
                                                       Convert_To_Real (Vals.Element (3))));
   end Handle_UIF;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Exponential_IDF (Convert_To_Real (Vals.Element (1)),
                                                           Convert_To_Real (Vals.Element (2))));
   end Handle_EIF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Beta_IDF (Convert_To_Real (Vals.Element (1)),
                                                    Convert_To_Real (Vals.Element (2)),
                                                    Convert_To_Real (Vals.Element (3))));
   end Handle_BIF;
   ...
   begin
      if not Has_Args (Vals, 1) then return (Kind => Val_Missing); end if;
      declare P : constant Real := Convert_To_Real (Vals.Element (1));
      begin
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Poisson_IDF (Convert_To_Real (Vals.Element (1)),
                                                       Convert_To_Real (Vals.Element (2))));
   end Handle_PIF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Gamma_IDF (Convert_To_Real (Vals.Element (1)),
                                                     Convert_To_Real (Vals.Element (2)),
                                                     Convert_To_Real (Vals.Element (3))));
   end Handle_GIF;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Chi_Square_IDF (Convert_To_Real (Vals.Element (1)),
                                                          Convert_To_Real (Vals.Element (2))));
   end Handle_XIF;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Student_T_IDF (Convert_To_Real (Vals.Element (1)),
                                                         Convert_To_Real (Vals.Element (2))));
   end Handle_TIF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.F_IDF (Convert_To_Real (Vals.Element (1)),
                                                 Convert_To_Real (Vals.Element (2)),
                                                 Convert_To_Real (Vals.Element (3))));
   end Handle_FIF;
   ...
   begin
      if not Has_Args (Vals, 3) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Weibull_IDF (Convert_To_Real (Vals.Element (1)),
                                                       Convert_To_Real (Vals.Element (2)),
                                                       Convert_To_Real (Vals.Element (3))));
   end Handle_WIF;
   ...
   begin
      if N = 0 then
         ...
         if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
         return Num_Result (SData_Core.Statistics.Normal_RN (Convert_To_Real (Vals.Element (1)),
                                                        Convert_To_Real (Vals.Element (2))));
      end if;
   end Handle_ZRN;
   ...
   begin
      if N = 0 then
         ...
         if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
         return Num_Result (SData_Core.Statistics.Uniform_RN (Convert_To_Real (Vals.Element (1)),
                                                         Convert_To_Real (Vals.Element (2))));
      end if;
   end Handle_URN;
   ...
   begin
      if not Has_Args (Vals, 1) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Exponential_RN (Convert_To_Real (Vals.Element (1))));
   end Handle_ERN;
   ...
   begin
      if not Has_Args (Vals, 1) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Poisson_RN (Convert_To_Real (Vals.Element (1))));
   end Handle_PRN;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Gamma_RN (Convert_To_Real (Vals.Element (1)),
                                                    Convert_To_Real (Vals.Element (2))));
   end Handle_GRN;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Binomial_RN (Convert_To_Real (Vals.Element (1)),
                                                       Convert_To_Real (Vals.Element (2))));
   end Handle_MRN;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Weibull_RN (Convert_To_Real (Vals.Element (1)),
                                                      Convert_To_Real (Vals.Element (2))));
   end Handle_WRN;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Beta_RN (Convert_To_Real (Vals.Element (1)),
                                                   Convert_To_Real (Vals.Element (2))));
   end Handle_BRN;
   ...
   begin
      if not Has_Args (Vals, 1) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Chi_Square_RN (Convert_To_Real (Vals.Element (1))));
   end Handle_XRN;
   ...
   begin
      if not Has_Args (Vals, 1) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.Student_T_RN (Convert_To_Real (Vals.Element (1))));
   end Handle_TRN;
   ...
   begin
      if not Has_Args (Vals, 2) then return (Kind => Val_Missing); end if;
      return Num_Result (SData_Core.Statistics.F_RN (Convert_To_Real (Vals.Element (1)),
                                                Convert_To_Real (Vals.Element (2))));
   end Handle_FRN;
